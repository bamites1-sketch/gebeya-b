      ...
      <div className="grid lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 space-y-3 sm:space-y-4">
          {items.map((item) => {
            ...
            return (
              <div key={item.id} className="flex gap-3 sm:gap-4 bg-white dark:bg-gray-800 rounded-2xl p-3 sm:p-4 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex-1 min-w-0">
                  <Link to={`/products/${item.product.id}`} className="font-semibold text-gray-900 dark:text-white hover:text-primary-600 transition-colors line-clamp-1 text-sm sm:text-base">
                    {item.product.name}
                  </Link>
                  <p className="text-xs text-gray-400 capitalize mt-0.5">{item.product.category}</p>
                  <p className="text-primary-600 font-bold mt-1 text-sm">{item.product.price.toLocaleString()} {t('common.etb')}</p>
                  <div className="flex items-center gap-2 sm:gap-3 mt-2 sm:mt-3">
                    <div className="flex items-center border dark:border-gray-600 rounded-xl overflow-hidden">
                      <button onClick={() => updateItem(item.id, item.quantity - 1)}
                        className="w-9 h-9 sm:w-11 sm:h-11 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-lg font-bold active:bg-gray-200">−</button>
                      <span className="w-8 sm:w-10 text-center text-sm font-semibold dark:text-white">{item.quantity}</span>
                      <button onClick={() => updateItem(item.id, item.quantity + 1)}
                        className="w-9 h-9 sm:w-11 sm:h-11 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-lg font-bold active:bg-gray-200">+</button>
                    </div>
                ...
                <div className="text-right shrink-0">
                  <p className="font-black text-gray-900 dark:text-white text-sm sm:text-base">{(item.product.price * item.quantity).toLocaleString()}</p>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 sm:p-6 shadow-sm lg:sticky lg:top-24">
            <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white mb-4 sm:mb-5">Order Summary</h2>
