...
export default function PaymentSuccessPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-xl sm:max-w-2xl mx-auto px-4 py-8 sm:py-12">
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl overflow-hidden">

        {/* Header */}
        <div className="relative text-white text-center py-7 sm:py-8 px-4 sm:px-6"
          ...
          <div className="relative">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
              ...
              className="w-14 h-14 sm:w-16 sm:h-16 bg-green-400 rounded-full flex items-center justify-center mx-auto mb-3 shadow-lg">
              <span className="text-2xl sm:text-3xl">✅</span>
            </motion.div>
            <h1 className="text-xl sm:text-2xl font-black">Payment Successful!</h1>
        ...
        <div className="p-4 sm:p-6">
          ...
          {/* Receipt header */}
          <div className="flex items-center justify-between mb-4 sm:mb-5 pb-4 border-b border-dashed border-gray-200">
            <div className="flex items-center gap-2 sm:gap-3">
              <img src="/logo.jpg" alt="gebeya-B" className="w-8 h-8 sm:w-10 sm:h-10 object-contain rounded-full"
                onError={(e) => { e.currentTarget.style.display = 'none'; }} />
              <div>
                <p className="font-black text-[#2C1810] text-xs sm:text-sm">gebeya-B</p>
                <p className="text-[9px] sm:text-[10px] text-gray-400">Ethiopian Cultural Marketplace</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-4 sm:mb-5 text-center">
            <div className="bg-[#F5F0E8] rounded-xl p-2.5 sm:p-3">
              <p className="text-[9px] sm:text-[10px] text-gray-400 mb-0.5">Date</p>
              <p className="text-xs font-bold text-[#2C1810]">{new Date().toLocaleDateString('en-GB')}</p>
            </div>
            <div className="bg-[#F5F0E8] rounded-xl p-2.5 sm:p-3">
              <p className="text-[9px] sm:text-[10px] text-gray-400 mb-0.5">Method</p>
              <p className="text-xs font-bold text-[#2C1810]">🔵 Chapa</p>
            </div>
            <div className="bg-[#F5F0E8] rounded-xl p-2.5 sm:p-3">
              <p className="text-[9px] sm:text-[10px] text-gray-400 mb-0.5">Status</p>
            ...
            <div className="mb-4">
              ...
              <div className="divide-y divide-gray-50">
                {order.items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between py-2 sm:py-2.5 text-sm">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <img
                        src={getFirstImage(item.product?.images, item.product?.name)}
                        alt={item.product?.name}
                        className="w-7 h-7 sm:w-8 sm:h-8 object-cover rounded-lg shrink-0"
                    ...
                    <span className="text-gray-400 text-xs mx-2 sm:mx-3">×{item.quantity}</span>
          ...
          {/* Totals */}
          {order?.totalPrice && (
            <div className="border-t border-dashed border-gray-200 pt-3 sm:pt-4 space-y-2">
              ...
              <div className="flex justify-between font-black text-base sm:text-lg text-[#2C1810] pt-2 border-t border-gray-200">
          ...
          {/* Transaction ref */}
          <div className="mt-3 sm:mt-4 bg-gray-50 rounded-xl p-2.5 sm:p-3 flex items-center justify-between gap-2">
            ...
            <span className="text-green-500 text-lg sm:text-xl shrink-0">🔒</span>
          </div>

          {/* Divider */}
          <div className="relative my-4 sm:my-5">
            <div className="border-t-2 border-dashed border-gray-200" />
            <div className="absolute -left-4 sm:-left-6 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 bg-[#F5F0E8] rounded-full" />
            <div className="absolute -right-4 sm:-right-6 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 bg-[#F5F0E8] rounded-full" />
          </div>

          <p className="text-center text-[10px] text-gray-400 mb-3 sm:mb-4">
      ...
      <div className="flex flex-col xs:flex-row gap-2 sm:gap-3 mt-4 sm:mt-6">
        <Link to="/"
          className="flex-1 text-center bg-[#F19A0E] hover:bg-[#d97b08] active:bg-[#c06d05] text-white py-3 sm:py-3.5 rounded-xl font-bold transition-colors text-sm sm:text-base active:scale-[0.98]">
          🏠 Back to Home
        </Link>
        <Link to="/profile"
          className="flex-1 text-center border-2 border-[#F19A0E] text-[#F19A0E] py-3 sm:py-3.5 rounded-xl font-bold hover:bg-[#FEF3E2] transition-colors text-sm sm:text-base active:scale-[0.98]">
          📦 View My Orders
        </Link>
      </div>
    </motion.div>
