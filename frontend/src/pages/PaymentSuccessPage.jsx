...
export default function PaymentSuccessPage() {
  ...
  useEffect(() => {
    if (!txRef) { setError('No transaction reference found.'); setLoading(false); return; }
    api.get(`/payments/verify/${txRef}`)
      .then(({ data }) => { setOrder(data.order); fetchCart(); })
  ...
  if (error) return (
    <div className="max-w-md mx-auto px-4 py-20 text-center">
      ...
      <Link to="/cart" className="bg-[#F19A0E] text-white px-6 py-3 rounded-xl font-bold hover:bg-[#d97b08] transition-colors">Back to Cart</Link>
    </div>
  );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto px-4 py-12">
      <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
        <div className="relative text-white text-center py-8 px-6" style={{ background: 'linear-gradient(135deg, #2C1810 0%, #1a0e06 50%, #078930 100%)' }}>
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-5 pb-4 border-b border-dashed border-gray-200">
            <div className="flex items-center gap-3">
              <img src="/logo.jpg" alt="gebeya-B" className="w-10 h-10 object-contain rounded-full" onError={(e) => { e.currentTarget.style.display='none'; }} />
              <div>
            ...
            <div className="text-right">
              ...
              <p className="font-mono text-xs font-bold text-[#2C1810]">#{order?.id}</p>
            </div>
          </div>
            ...
            <div className="divide-y divide-gray-50">
              {order?.items?.map((item) => (
                <div key={item.id} className="flex items-center justify-between py-2.5 text-sm">
                  ...
                  <span className="font-bold text-[#2C1810] text-xs shrink-0">{(item.price * item.quantity).toLocaleString()} ETB</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border-t border-dashed border-gray-200 pt-4 space-y-2">
            <div className="flex justify-between text-sm text-gray-500"><span>Subtotal</span><span>{order?.totalPrice?.toLocaleString()} ETB</span></div>
          ...
          <p className="text-center text-[10px] text-gray-400 mb-4">🇪🇹 gebeya-B · Powered by Chapa · Thank you for supporting Ethiopian artisans</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-6">
        <Link to="/" className="flex-1 text-center bg-[#F19A0E] hover:bg-[#d97b08] text-white py-3.5 rounded-xl font-bold transition-colors">🏠 Back to Home</Link>
        <Link to="/profile" className="flex-1 text-center border-2 border-[#F19A0E] text-[#F19A0E] py-3.5 rounded-xl font-bold hover:bg-[#FEF3E2] transition-colors">📦 View My Orders</Link>
      </div>
    </motion.div>
  );
}
